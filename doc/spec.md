...
| `src/constraint/` | 2473 | 拘束ヤコビアン・等式/不等式拘束付き IK（5 サブモジュール） |
| ├ `types.rs` | 200 | 型定義（`ConstraintType`, `RigidConstraint`, `ConstraintModel`） |
| ├ `error.rs` | 239 | 拘束誤差計算 |
| ├ `jacobian.rs` | 428 | 拘束ヤコビアン計算 |
| ├ `ik.rs` | 576 | DLS ベース拘束付き IK ソルバー |
| ├ `qp_ik.rs` | 927 | QP ベース不等式拘束付き IK ソルバー |
...
### 4.27 拘束ヤコビアン・拘束付き IK (`constraint/`)

Pinocchio 互換の拘束ヤコビアンフレームワーク。ループ閉鎖、クロスブランチ IK、相対姿勢拘束を提供する。
5 つのサブモジュールに分割し、`mod.rs` でフラットに再エクスポートするため、外部 API は変更なし（`use misarta::constraint::{...}`）。

#### 4.27.1 型定義 (`constraint/types.rs`)
...
#### 4.27.2 拘束誤差 (`constraint/error.rs`)
...
| `compute_constraint_error_from_data` | FK データ再利用版 |

#### 4.27.3 拘束ヤコビアン (`constraint/jacobian.rs`)
...
| `compute_constraint_jacobian_from_data` | FK データ再利用版 |

#### 4.27.4 DLS 拘束 IK (`constraint/ik.rs`)
...
#### 4.27.5 QP ベース不等式拘束 IK (`constraint/qp_ik.rs`)
...
| 型 / 関数 | 説明 |
|------|------|
| `QpIkConfig` | QP ベース不等式拘束 IK 設定（`qp_solver` フィールドでバックエンド選択可） |
