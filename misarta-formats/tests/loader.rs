...
fn load_fixture_urdf() {
    ...
    let model = misarta_formats::urdf::load_urdf(&path).expect("failed to load fixture URDF");
...
fn urdf_fixture_fk_zero() {
    let path = fixture_path("urdf/test_robot.urdf");
    let model = misarta_formats::urdf::load_urdf(&path).unwrap();
...
fn load_fixture_sdf() {
    ...
    let model = misarta_formats::sdf::load_sdf(&path).expect("failed to load fixture SDF");
...
fn sdf_fixture_fk_zero() {
    let path = fixture_path("sdf/test_robot.sdf");
    let model = misarta_formats::sdf::load_sdf(&path).unwrap();
...
fn urdf_and_sdf_produce_same_fk() {
    ...
    let urdf_model = misarta_formats::urdf::load_urdf(&fixture_path("urdf/test_robot.urdf")).unwrap();
    let sdf_model = misarta_formats::sdf::load_sdf(&fixture_path("sdf/test_robot.sdf")).unwrap();
...
fn urdf_reload_produces_identical_model() {
    ...
    let path = fixture_path("urdf/test_robot.urdf");
    let m1 = misarta_formats::urdf::load_urdf(&path).unwrap();
    let m2 = misarta_formats::urdf::load_urdf(&path).unwrap();
...
fn sdf_reload_produces_identical_model() {
    let path = fixture_path("sdf/test_robot.sdf");
    let m1 = misarta_formats::sdf::load_sdf(&path).unwrap();
    let m2 = misarta_formats::sdf::load_sdf(&path).unwrap();
    assert!(m1.approx_eq(&m2, 1e-14));
}
...
fn urdf_fixture_roundtrip() {
    ...
    let path = fixture_path("urdf/test_robot.urdf");
    let m1 = misarta_formats::urdf::load_urdf(&path).unwrap();
    let xml = misarta_formats::urdf::write_urdf_string(&m1);
    let m2 = misarta_formats::urdf::load_urdf_string(&xml).unwrap();
...
fn sdf_fixture_roundtrip() {
    ...
    let path = fixture_path("sdf/test_robot.sdf");
    let m1 = misarta_formats::sdf::load_sdf(&path).unwrap();
    let xml = misarta_formats::sdf::write_sdf_string(&m1);
    let m2 = misarta_formats::sdf::load_sdf_string(&xml).unwrap();
...
fn urdf_to_sdf_cross_roundtrip() {
    ...
    let urdf_model = misarta_formats::urdf::load_urdf(&fixture_path("urdf/test_robot.urdf")).unwrap();
    let sdf_xml = misarta_formats::sdf::write_sdf_string(&urdf_model);
    let sdf_model = misarta_formats::sdf::load_sdf_string(&sdf_xml).unwrap();
    assert!(
        urdf_model.approx_eq(&sdf_model, 1e-10),
...
fn sdf_to_urdf_cross_roundtrip() {
    ...
    let sdf_model = misarta_formats::sdf::load_sdf(&fixture_path("sdf/test_robot.sdf")).unwrap();
    let urdf_xml = misarta_formats::urdf::write_urdf_string(&sdf_model);
    let urdf_model = misarta_formats::urdf::load_urdf_string(&urdf_xml).unwrap();
    assert!(
...
fn urdf_fixture_load_geometry() {
    let path = fixture_path("urdf/test_robot.urdf");
    let (model, vis, col) = misarta_formats::urdf::load_urdf_geometry(&path).unwrap();
...
fn sdf_fixture_load_geometry() {
    let path = fixture_path("sdf/test_robot.sdf");
    let (model, vis, col) = misarta_formats::sdf::load_sdf_geometry(&path).unwrap();
...
fn urdf_fixture_geometry_roundtrip() {
    let path = fixture_path("urdf/test_robot.urdf");
    let (model, vis, col) = misarta_formats::urdf::load_urdf_geometry(&path).unwrap();
    let xml = misarta_formats::urdf::write_urdf_geometry_string(&model, Some(&vis), Some(&col));
    let (model2, vis2, col2) = misarta_formats::urdf::load_urdf_geometry_string(&xml).unwrap();
...
fn sdf_fixture_geometry_roundtrip() {
    let path = fixture_path("sdf/test_robot.sdf");
    let (model, vis, col) = misarta_formats::sdf::load_sdf_geometry(&path).unwrap();
    let xml = misarta_formats::sdf::write_sdf_geometry_string(&model, Some(&vis), Some(&col));
    let (model2, vis2, col2) = misarta_formats::sdf::load_sdf_geometry_string(&xml).unwrap();
...
fn urdf_sdf_geometry_shapes_match() {
    ...
    let (_, urdf_vis, _) = misarta_formats::urdf::load_urdf_geometry(&fixture_path("urdf/test_robot.urdf")).unwrap();
    let (_, sdf_vis, _) = misarta_formats::sdf::load_sdf_geometry(&fixture_path("sdf/test_robot.sdf")).unwrap();
