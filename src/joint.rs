...
use nalgebra::{Matrix6xX, Translation3, UnitQuaternion, Vector3};
...
impl JointType {
    ...
    pub fn forward(&self, q: &[f64]) -> SE3 {
        match self {
            ...
            JointType::Prismatic { axis } => {
                let d = q[0];
                SE3::from_parts(Translation3::from(axis * d), UnitQuaternion::identity())
            }
            JointType::Fixed => se3::identity(),
            JointType::FreeFlyer => {
                // q = [x, y, z, qx, qy, qz, qw]
                let t = Translation3::new(q[0], q[1], q[2]);
                let quat =
    ...
    pub fn motion_subspace(&self, _q: &[f64]) -> Matrix6xX<f64> {
        match self {
            JointType::Revolute { axis } => {
